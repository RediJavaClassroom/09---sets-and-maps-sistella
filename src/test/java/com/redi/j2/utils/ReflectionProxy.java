...
public abstract class ReflectionProxy {
    ...
    public boolean isPropertyOfType(String name, Class<?> type, Class<?>... parameterizedTypes) {
        Class<?> targetClass = getTargetClass();
        ...
        try {
            ...
            if (pType.getActualTypeArguments().length != parameterizedTypes.length) {
                return false;
            }
            for(int i=0; i<parameterizedTypes.length; i++) {
                if (!pType.getActualTypeArguments()[i].equals(parameterizedTypes[i])) {
                    return false;
                }
            }
            return true;
